...
router.put("/addresses/:id", verifyToken , async(req, res) => {
    try{
      ...
      if(foundAddress) {
        ...
        if(req.body.streetAdress) foundAddress.streetAdress  = req.body.streetAdress;
...
router.put("/addresses/set/default", verifyToken, async(req, res) => {
  try{
    ...
    const setAddressDefault = await User.findOneAndUpdate({ _id: req.decoded._id }, { $set: {address : req.body.id }});
    if(setAddressDefault){
      res.json({status : true,message: 'Successfully set this address as default'});
    }
...
router.delete("/addresses/:id", verifyToken, async(req, res) => {
  try{
    const deleteAddress = await Address.remove({ user: req.decoded._id , _id: req.params.id });
